...
//    private Gamefield gf =
     ...
     * @param eingabex the x Position of the Quadrant that will have the given
     * Value removed
     * @param eingabey the x Position of the Quadrant that will have the given
     * Value removed
     */
    ...
    public void findHorizontalUniqueNumbers(Gamefield gf, int row) {
        HashMap<Integer, Integer> numberInMatrix = new HashMap<>();
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < background[row][i].size(); j++) {
                Integer zahl = background[row][i].get(j);
                if (numberInMatrix.containsKey(zahl)) {
                    numberInMatrix.put(zahl, numberInMatrix.get(zahl) + 1);
                } else {
                    numberInMatrix.put(zahl, 1);
                }
            }
        }
        for (int i = 0; i < 9; i++) {
    ...
    public void findVerticalUniqueNumbers(Gamefield gf, int col) {
        HashMap<Integer, Integer> numberInMatrix = new HashMap<>();//First is the zahl, second is the count
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < background[i][col].size(); j++) {
                Integer zahl = background[i][col].get(j);
                if (numberInMatrix.containsKey(zahl)) {
                    numberInMatrix.put(zahl, numberInMatrix.get(zahl) + 1);
                } else {
                    numberInMatrix.put(zahl, 1);
            ...
            for (int j = outputy - 1; j < outputy + 2; j++) {
                if (i < 9 && j < 9) {
                    for (int k = 0; k < background[i][j].size(); k++) {
                        Integer zahl = background[i][j].get(k);
                        if (numberInMatrix.containsKey(zahl)) {
                            numberInMatrix.put(zahl, numberInMatrix.get(zahl) + 1);
                        } else {
                            numberInMatrix.put(zahl, 1);
    ...
    public boolean isSolved(Gamefield gf) {
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                if (gf.getValOnField(i, j) == 0) {
                    return false;
